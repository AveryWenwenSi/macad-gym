...
class Reward(object):
    ...
    def compute_reward_hiway_lane_change(self):
        ...
        if new_damage:
            self.reward -= 1          #?? Maybe set collision damage smaller, the cars can behave more aggresively? Since they are collecting the most reward
        ...
        #    self.reward += np.clip(prev_dist_y - cur_dist_y, -10.0, 10.0) * 0.5
        #else:
        #    self.reward += np.clip(prev_dist_y - cur_dist_y, -10.0, 10.0) * 1.0
